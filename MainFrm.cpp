...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	// 도킹 팬 추가
	if (!m_paneTextOut.Create(_T("문자열 출력"), this, CRect(0, 0, 300, 300), TRUE, 0xfafa,
		WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | CBRS_RIGHT | CBRS_FLOAT_MULTI))
	{
		MessageBox(_T("도킹 팬 에러"));
		return -1;
	}
	m_paneTextOut.SetMinSize(300);
	m_paneTextOut.EnableDocking(CBRS_ALIGN_ANY);
	DockPane(&m_paneTextOut);
